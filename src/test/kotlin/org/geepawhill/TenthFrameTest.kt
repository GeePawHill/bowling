...
class TenthFrameTest {
    ...
    fun `tenth rolls update ball fields`() {
        tenth.isFilledAfterScoring(0, Roll.Four)
        tenth.isFilledAfterScoring(0, Roll.Spare)
        tenth.isFilledAfterScoring(0, Roll.Five)
        assertThat(tenth.third.value).isEqualTo("5")
    }

    @Test
    fun `tenth rolls no mark advance frame`() {
        assertThat(tenth.isFilledAfterScoring(0, Roll.Four)).isFalse()
        assertThat(tenth.isFilledAfterScoring(0, Roll.Five)).isTrue()
    }

    @Test
    fun `tenth rolls strike does not advance frame`() {
        assertThat(tenth.isFilledAfterScoring(0, Roll.Strike)).isFalse()
    }

    @Test
    fun `tenth rolls spare does not advance frame`() {
        assertThat(tenth.isFilledAfterScoring(0, Roll.Four)).isFalse()
        assertThat(tenth.isFilledAfterScoring(0, Roll.Spare)).isFalse()
    }
    ...
    fun `tenth third roll advances frame`() {
        assertThat(tenth.isFilledAfterScoring(0, Roll.Four)).isFalse()
        assertThat(tenth.isFilledAfterScoring(0, Roll.Spare)).isFalse()
        assertThat(tenth.isFilledAfterScoring(0, Roll.Four)).isTrue()
    }
    ...
    fun `complete score with strike and 2 other rolls`() {
        tenth.isFilledAfterScoring(0, Roll.Strike)
        tenth.isFilledAfterScoring(0, Roll.Four)
        tenth.isFilledAfterScoring(0, Roll.Five)
